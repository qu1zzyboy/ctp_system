...
| 模块 | 文件 | 职责 |
|------|------|------|
| Actor | `actor.rs` | `ServerActorState`、Client 注册、账户绑定、指令分发、回报推送 |
| Trade 命令 | `trade_actor_ops.rs` | 撤单、四类查询、登出的 CTP 驱动逻辑 |
...
sequenceDiagram
    participant C as Client
    participant O as trade_actor_ops
    participant CTP as CTP Trade
